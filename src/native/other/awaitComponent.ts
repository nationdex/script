...
export default new NativeFunction({
    ...
    args: [
        ...
        },
        {
            name: "filter",
            description:
                "The filter to run for every interaction received after this, this is called with interaction context",
            rest: false,
            required: true,
            condition: true,
            type: ArgType.String,
    ...
    async execute(ctx): Promise<Return> {
        ...
        if (int) {
            const rt = await this["resolveCode"](
                ctx.clone({ obj: int }),
                this.data.fields![3] as IExtendedCompiledFunctionField
            )
            if (!this["isValidReturnType"](rt)) return rt
        }
